...
LIBTCCAPI int tcc_relocate(TCCState *s1, void *ptr)
{
    ...
    { /* Use mmap instead of malloc for Selinux. */
        ...
        s1->write_mem = mmap(NULL, ret, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
        if (s1->write_mem == MAP_FAILED)
            tcc_error("mmap not writeable");

        s1->runtime_mem = mmap(NULL, ret, PROT_READ | PROT_EXEC, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
