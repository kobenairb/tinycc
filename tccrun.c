...
LIBTCCAPI int tcc_relocate(TCCState *s1)
{
    ...
    if ((ret = tcc_relocate_ex(s1, NULL)) < 0)
        return -1;
    s1->mem_size = ret;
    unlink(tmpfname);
    ftruncate(fd, s1->mem_size);
    s1->write_mem = mmap(NULL, ret, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
    if (s1->write_mem == MAP_FAILED) {
        tcc_error("/tmp not writeable");
        return -1;
    }
    s1->runtime_mem = mmap(NULL, ret, PROT_READ | PROT_EXEC, MAP_SHARED, fd, 0);
...
static void win64_add_function_table(TCCState *s1)
{
    RtlAddFunctionTable((RUNTIME_FUNCTION *) (uplong) s1->uw_pdata->sh_addr,
