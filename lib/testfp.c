...
{
    u128_t x = {low,
                (0x0000ffffffffffff & high) | (0x7fff000000000000 & (uint64_t) exp << 48)
...
void adds(void)
{
    ...
    // Check normalisation:
    {
        ...
        for (e = 0; e < sizeof(ee) / sizeof(*ee); e++) {
            ...
            for (i = 63; i >= 0; i--)
                fsub(make(0, exp, a1 | (uint64_t) 1 << i >> 1, a0), make(0, exp, a1 >> i << i, 0));
            for (i = 63; i >= 0; i--)
                fsub(make(0, exp, a1, a0 | (uint64_t) 1 << i >> 1), make(0, exp, a1, a0 >> i << i));
        }
    }
...
void muls(void)
{
    ...
    for (i = 117; i > 0; i--)
        fmul(make(0, 16268, 0x643dcea76edc, 0xe0877a598403627a), make(i & 1, i, 0, 0));
