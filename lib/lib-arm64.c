...
static int fp3_detect_NaNs(
    long double *f, int a_sgn, int a_exp, u128_t a, int b_sgn, int b_exp, u128_t b)
{
...
long double __extendsftf2(float f)
{
    ...
    if (!(a << 1))
        ...
        x.x1 = (0x7fff000000000000 | aa >> 31 << 63 | aa << 41 >> 16 | (uint64_t) !!(a << 9) << 47);
    else
...
long double __extenddftf2(double f)
{
    ...
    if (!(a << 1))
        ...
        x.x1 = (0x7fff000000000000 | a >> 63 << 63 | a << 12 >> 16 | (uint64_t) !!(a << 12) << 47);
    else
...
double __trunctfdf2(long double f)
{
    ...
    if (exp == 32767 && (mnt.x0 | mnt.x1 << 16))
        x = (0x7ff8000000000000 | (uint64_t) sgn << 63 | mnt.x1 << 16 >> 12 | mnt.x0 >> 60);
    else if (exp > 17406)
...
long double __floatsitf(int32_t a)
{
    ...
    if (a) {
        ...
        x.x1 = ((uint64_t) sgn << 63 | (uint64_t) exp << 48 | (uint64_t) (mnt << 1) << 16);
    }
    memcpy(&f, &x, 16);
    return f;
}
