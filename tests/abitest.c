...
static int ret_2float_test_callback(void *ptr)
{
    ...
    r = f(a);
    return ((r.x == a.x * 5) && (r.y == a.y * 3) && (f(a).x == a.x * 5) && (f(a).y == a.y * 3))
               ? 0
               : -1;
}
...
typedef struct ret_longdouble_test_type_s2
{
    LONG_DOUBLE x;
} ret_longdouble_test_type;
...
typedef struct ret_longlong_test_type_s2
{
    int x[4];
} ret_longlong_test_type;
...
{
    const char *src = "typedef struct ret_longlong_test_type_s2 {int x[4];} ret_longlong_test_type;"
