...
#define CHARACTERISTICS_DLL 0x230E
    ...
    pe->sizeofheaders = pe_file_align(sizeof(struct pe_header)
    ...
    for (i = 0; i < pe->sec_count; ++i) {
        ...
        pe_header.opthdr.SizeOfImage = umax(pe_virtual_align(size + addr),
...
ST_FN void pe_build_imports(struct pe_info *pe)
{
    ...
    for (i = 0; i < pe->imp_count; ++i) {
        ...
        for (k = 0, n = p->sym_count; k <= n; ++k) {
            ...
            *(ADDR3264 *) (pe->thunk->data + thk_ptr) = *(ADDR3264 *) (pe->thunk->data + ent_ptr)
                = v;
...
ST_FN int pe_load_res(TCCState *s1, FILE *fp)
{
    ...
    if (hdr.filehdr.Machine != 0x014C || hdr.filehdr.NumberOfSections != 1
    ...
    for (i = 0; i < hdr.sectionhdr.NumberOfRelocations; ++i) {
        ...
        if (rel.type != 7) /* DIR32NB */
            goto quit;
        put_elf_reloc(symtab_section, rsrc_section, rel.offset, R_XXX_RELATIVE, 0);
        offs += sizeof rel;
    }
    ret = 0;
quit:
    ...
    for (;;) {
        ...
        default:
            add_elf_sym(s1->dynsymtab_section,
                        s1->nb_loaded_dlls,
                        0,
                        ELFW_ST_INFO(STB_GLOBAL, STT_FUNC),
                        0,
                        text_section->sh_num,
                        p);
            continue;
...
ST_FN void pe_add_runtime_ex(TCCState *s1, struct pe_info *pe)
{
    ...
    start_symbol = TCC_OUTPUT_MEMORY == s1->output_type ? PE_GUI == pe_type ? "_runwinmain" : NULL
                   : PE_DLL == pe_type                  ? PE_STDSYM("_dllstart", "@12")
                   : PE_GUI == pe_type                  ? "_winstart"
                                                        : "_start";
    ...
    if (0 == s1->nostdlib) {
        static const char *libs[] = {"tcc1", "msvcrt", "kernel32", "", "user32", "gdi32", NULL};
        const char **pp, *p;
