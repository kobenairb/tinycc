...
#define DEF_ASM_OP2(name, opcode, group, instr_type, op0, op1) \
    {TOK_ASM_##name, opcode, (instr_type | group << OPC_GROUP_SHIFT), 2, {op0, op1}},
#define DEF_ASM_OP3(name, opcode, group, instr_type, op0, op1, op2) \
    {TOK_ASM_##name, opcode, (instr_type | group << OPC_GROUP_SHIFT), 3, {op0, op1, op2}},
#ifdef TCC_TARGET_X86_64
...
void printf_asm_opcode()
{
    ...
    printf("size=%d nb=%d f0=%d f1=%d f2=%d f3=%d\n",
           ...
           (int) sizeof(asm_instrs) / sizeof(ASMInstr),
           freq[0],
           freq[1],
           freq[2],
           freq[3]);
...
static void gen_disp16(ExprValue *pe)
{
    ...
    if (sym) {
        ...
        put_elf_reloc(symtab_section, cur_text_section, ind, R_386_PC16, 0);
        gen_le16(pe->v - 2);
    }
}
#endif
...
ST_FUNC void asm_opcode(TCCState *s1, int opcode)
{
    ...
    if (pa->sym == 0) {
        if (opcode >= TOK_ASM_first && opcode <= TOK_ASM_last) {
            ...
            tcc_error("unknown opcode '%s'", get_tok_str(opcode, NULL));
        }
    }
    ...
    if (s == 1 || (pa->instr_type & OPC_D16))
        ...
        /* generate REX prefix */
        if ((opcode != TOK_ASM_push && opcode != TOK_ASM_pop) || !(ops[0].type & OP_REG64))
            g(0x48);
    }
#endif
#endif
