...
PUB_FUNC int tcc_split_path(const char *in, const char *const *prefixs, int nb_prefixs, char ***out)
{
    int i, nb_comps = 0;
    char *path;
    const char *end;
    ...
    *out = NULL;
    do {
        ...
        for (i = 0; i < nb_prefixs; i++) {
            size = (strlen(prefixs[i]) + 1) * sizeof(char) + (end - in);
...
#undef str
...
#ifdef CONFIG_TCC_EXTRA_LDDIR
    {
        ...
        nb_extra_lddirs
            = tcc_split_path(CONFIG_TCC_EXTRA_LDDIR, lddir_prefixs, nb_prefixs, &extra_lddirs);
...
LIBTCCAPI int tcc_set_output_type(TCCState *s, int output_type)
{
    ...
    if (!s->nostdinc) {
...
#ifndef TCC_TARGET_PE
        {
            ...
            const char *const incdir_prefixs[] = {incdir_prefix1, incdir_prefix2};

            nb_prefixs = sizeof incdir_prefixs / sizeof *incdir_prefixs;
            nb_extra_incdirs
                = tcc_split_path(CONFIG_TCC_INCSUBDIR, incdir_prefixs, nb_prefixs, &extra_incdirs);
