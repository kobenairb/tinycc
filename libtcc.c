...
static int tcc_add_library_internal(
    TCCState *s, const char *fmt, const char *filename, int flags, char **paths, int nb_paths)
{
    char buf[1024];
    int i;
...
{
    return tcc_add_library_internal(s,
                                    "%s/%s",
                                    filename,
                                    flags,
                                    s->library_paths,
...
static void tcc_normalize_inc_dirs_aux(file_info_t *stats, size_t *pnum, char **path)
{
    size_t i, num = *pnum;
...
{
    file_info_t *stats = tcc_malloc(((size_t) s->nb_sysinclude_paths + s->nb_include_paths)
                                    * sizeof(*stats));
    size_t i, num = 0;
...
ST_FUNC void args_parser_add_listfile(TCCState *s, const char *filename)
{
    ...
    for (ch = handle_eob(), args_parser_trim_ws();; inp()) {
        ...
        if (ch == '\n' || ch == '\r' || ch == CH_EOF || pb - buf >= sizeof(buf) - 1) {
            if (pb > buf) {
                *pb = 0, pb = buf;
