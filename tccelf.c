...
ST_FUNC void relocate_section(TCCState *s1, Section *s)
{
    ...
    for_each_elem(sr, 0, rel, ElfW_Rel)
    {
        ...
        if (relocs_info[type].gotplt_entry != NO_GOTPLT_ENTRY && relocs_info[type].code_reloc
            && symattr && symattr->plt_offset)
...
ST_FUNC void build_got_entries(TCCState *s1)
{
    ...
    for (i = 1; i < s1->nb_sections; i++) {
        ...
        for_each_elem(s, 0, rel, ElfW_Rel)
        {
            ...
            if (sym->st_shndx == SHN_UNDEF && s1->output_type == TCC_OUTPUT_DLL
                && !PCRELATIVE_DLLPLT && relocs_info[type].gotplt_entry == AUTO_GOTPLT_ENTRY)
                continue;
            ...
            put_got_entry(s1, reloc_type, type, sym->st_size, sym->st_info, sym_index);
        }
    }
}
