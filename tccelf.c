...
ST_FUNC void relocate_section(TCCState *s1, Section *s)
{
    ...
    for_each_elem(sr, 0, rel, ElfW_Rel)
    {
        ...
        case R_AARCH64_LD64_GOT_LO12_NC:
            write32le(ptr,
                      ((read32le(ptr) & 0xfff803ff)
                       | ((s1->got->sh_addr + s1->sym_attrs[sym_index].got_offset) & 0xff8) << 7));
            break;
        case R_AARCH64_COPY:
            break;
        ...
        case R_X86_64_GOTPCREL:
            write32le(ptr,
                      read32le(ptr)
                          + (s1->got->sh_addr - addr + s1->sym_attrs[sym_index].got_offset - 4));
            break;
...
ST_FUNC void relocate_plt(TCCState *s1)
{
    ...
    if (p < p_end) {
        ...
        while (p < p_end) {
            ...
            uint64_t addr = got + (read32le(p) | (uint64_t) read32le(p + 4) << 32);
