...
ST_FUNC void squeeze_multi_relocs(Section *s, size_t oldrelocoffset)
{
    ...
    for (a = oldrelocoffset + sizeof(*r); a < sr->data_offset; a += sizeof(*r)) {
        ...
        for (; i >= (ssize_t) oldrelocoffset && ((ElfW_Rel *) (sr->data + i))->r_offset > addr;
             i -= sizeof(*r)) {
...
static struct sym_attr *put_got_entry(
    TCCState *s1, int dyn_reloc_type, int reloc_type, unsigned long size, int info, int sym_index)
{
    int need_plt_entry;
    const char *name;
    ElfW(Sym) * sym;
    ...
    if (s1->dynsym) {
        if (ELFW(ST_BIND)(sym->st_info) == STB_LOCAL) {
           ...
           got is just normal data).  */
            ...
            if (0 == attr->dyn_index)
                attr->dyn_index
                    = set_elf_sym(s1->dynsym, sym->st_value, size, info, 0, sym->st_shndx, name);
    ...
    if (need_plt_entry) {
        ...
        attr->plt_sym = put_elf_sym(s1->symtab,
                                    attr->plt_offset,
                                    sym->st_size,
    ...
    for (i = 1; i < s1->nb_sections; i++) {
        ...
        for_each_elem(s, 0, rel, ElfW_Rel)
        {
           ...
           targets might be too far from application code.  */
            ...
            attr = put_got_entry(s1, reloc_type, type, sym->st_size, sym->st_info, sym_index);
        ...
        if (sym->st_shndx == SHN_UNDEF) {
            ...
            if (sym_index) {
                ...
                if ((type == STT_FUNC) || (type == STT_GNU_IFUNC)) {
                     ...
                     * address */
                    int dynindex = put_elf_sym(s1->dynsym,
                                               0,
                                               esym->st_size,
                                               ELFW(ST_INFO)(STB_GLOBAL, STT_FUNC),
                                               0,
...
static void tidy_section_headers(TCCState *s1, int *sec_order)
{
    ...
    for_each_elem(symtab_section, 1, sym, ElfW(Sym)) if (sym->st_shndx != SHN_UNDEF
...
static int elf_output_file(TCCState *s1, const char *filename)
{
    ...
    if (file_type != TCC_OUTPUT_OBJ) {
        ...
        if (!s1->static_link) {
            ...
            if (s1->rpath)
                put_dt(dynamic,
                       s1->enable_new_dtags ? DT_RUNPATH : DT_RPATH,
