...
ST_FUNC int put_elf_sym(
    Section *s, uplong value, unsigned long size, int info, int other, int shndx, const char *name)
{
    ...
    for (rel = qrel; rel < rel_end; rel++) {
        ...
        case R_X86_64_PLT32: {
            ...
            if (diff <= -2147483647 || diff > 2147483647) {
                ...
                if (s1->output_type == TCC_OUTPUT_MEMORY) {
                    val = (add_jmp_table(s1, val - rel->r_addend) + rel->r_addend);
                    diff = val - addr;
                }
#endif
                if (diff <= -2147483647 || diff > 2147483647) {
...
#endif
            *(int *) ptr += (s1->got->sh_addr - addr + (s1->sym_infos[sym_index].got_offset << 1)
                             - 4);
            break;
...
static void put_sym_info(TCCState *s1, int index)
{
    ...
    if (index >= s1->nb_sym_infos) {
        ...
        s1->sym_infos = tab;
        memset(s1->sym_infos + s1->nb_sym_infos, 0, (n - s1->nb_sym_infos) * sizeof(unsigned long));
        s1->nb_sym_infos = n;
    }
}
...
static void put_got_entry(TCCState *s1, int reloc_type, unsigned long size, int info, int sym_index)
{
    ...
    if (sym_index < s1->nb_sym_infos && s1->sym_infos[sym_index].got_offset << 1)
        return;
