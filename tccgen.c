...
ST_DATA int
    vla_sp_loc; /* Pointer to variable holding location to store stack pointer on the stack when modifying stack pointer */
...
ST_FUNC void vstore(void)
{
    ...
    if ((((sbt == VT_INT || sbt == VT_SHORT) && dbt == VT_BYTE)
        ...
        vtop->type.t = (ft & VT_TYPE & ~VT_BITFIELD & ((1 << VT_STRUCT_SHIFT) - 1));
        /* XXX: factorize */
        if (ft & VT_CONSTANT)
            tcc_warning("assignment of read-only location");
    } else {
...
static void struct_layout(CType *type, AttributeDef *ad)
{
    ...
    for (f = type->ref->next; f; f = f->next) {
        ...
        if (extra_bytes)
            ...
            if (pcc)
                size = (bit_size + 7) >> 3;
            f->type.t = (f->type.t & ~(0x3f << VT_STRUCT_SHIFT)) | (0 << VT_STRUCT_SHIFT);
            goto union_tail;
        } else {
           ...
           placed adjacent.  */
            ...
            (bit_pos + bit_size > size * 8)
                ...
                if (bit_size == 0
                    || (typealign != 1
                        && (ofs2 / (typealign * 8)) > ((size * 8) / (typealign * 8)))) {
           ...
           of the containing struct using the base types alignment,
                ...
                if (((bit_pos + bit_size > size * 8) || (bit_size == 0 && prevbt == bt)
    ...
    printf("\n");
        ...
        if (f->v & SYM_FIRST_ANOM && (f->type.t & VT_BTYPE) == VT_STRUCT) {
           ...
           them.  */
                ass = f->type.ref;
                f->type.ref
                    = sym_push(anon_sym++ | SYM_FIELD, &f->type.ref->type, 0, f->type.ref->c);
                pps = &f->type.ref->next;
    ...
    type->ref->c = (c + (pcc ? (bit_pos + 7) >> 3 : 0) + maxalign - 1) & -maxalign;
    type->ref->r = maxalign;
}
...
static int parse_btype(CType *type, AttributeDef *ad)
{
    ...
    while (1) {
        ...
        default:
            ...
            type->t = ((s->type.t & ~VT_TYPEDEF) | (t & ~(VT_CONSTANT | VT_VOLATILE)));
            type->ref = s->type.ref;
...
static int condition_3way(void)
{
    int c = -1;
    if ((vtop->r & (VT_VALMASK | VT_LVAL)) == VT_CONST
        && (!(vtop->r & VT_SYM) || !(vtop->sym->type.t & VT_WEAK))) {
        CType boolean;
        boolean.t = VT_BOOL;
        vdup();
        gen_cast(&boolean);
...
static void decl_designator(
    CType *type, Section *sec, unsigned long c, Sym **cur_field, int size_only)
{
    Sym *s, *f;
    ...
    if (have_elem && !(type->t & VT_ARRAY) &&
       ...
       of course assignable to non-const elements.  */
        ...
        do_init_list:
           ...
           ensures that it even works with designators) */
                if (!size_only && array_length < index) {
                    init_putz(sec, c + array_length * size1, (index - array_length) * size1);
                }
                if (type->t & VT_ARRAY) {
        ...
        if (!size_only && array_length < n) {
            init_putz(sec, c + array_length * size1, (n - array_length) * size1);
        }
        if (!no_oblock)
            skip('}');
...
static void func_decl_list(Sym *func_sym)
{
    ...
    while (tok != '{' && tok != ';' && tok != ',' && tok != TOK_EOF && tok != TOK_ASM1
           && tok != TOK_ASM2 && tok != TOK_ASM3) {
           ...
           that size.  */
            ...
            if (tok == '{') {
                ...
                if (sym) {
                    ...
                    if (ref->a.func_call != FUNC_CDECL && type.ref->a.func_call == FUNC_CDECL)
