                ...
                if (bit_size == 0
                    ...
                    if (bit_pos + bit_size > size * 8) {
                        ...
                        if ((ad->a.packed || f->r) && packwarn) {
                            tcc_warning(
                                "struct layout not compatible with GCC (internal limitation)");
                            packwarn = 0;
                        }
                    }
                }
...
tok_next:
    ...
    case TOK_GENERIC: {
        ...
        if (!has_match && !has_default) {
            ...
            tcc_error("_Generic sellector of type '%s' is not compatible with any assosiation", buf);
        }
        skip(')');
...
static int decl_designator(
    CType *type, Section *sec, unsigned long c, Sym **cur_field, int size_only, int al)
{
    Sym *s, *f;
