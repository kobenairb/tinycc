...
static void gcall_or_jmp(int is_jmp)
{
    ...
    if ((vtop->r & (VT_VALMASK | VT_LVAL)) == VT_CONST
        ...
        if (vtop->r & VT_SYM) {
            ...
            put_elf_reloc(symtab_section, cur_text_section, ind + 1, R_X86_64_PC32, 0);
        }
    ...
    CType *vt, int variadic, CType *ret, int *ret_align, int *regsize, RegArgs *args)
{
    ...
    *regsize = 8;
    ...
    return mode != x86_64_mode_memory && mode != x86_64_mode_none;
}
        ...
        case x86_64_mode_sse:
            ...
            if (!arg_stored) {
                ...
                for (i = 0; i < reg_count_integer; ++i) {
                    gen_modrm64(0x89,
                                arg_regs[reg_param_index],
                                VT_LOCAL,
                                NULL,
                                param_addr + args.ireg[i]);
                    ++reg_param_index;
                }
            } else {
...
void gfunc_epilog(void)
{
    ...
    if (tcc_state->do_bounds_check && func_bound_offset != lbounds_section->data_offset) {
        ...
        *bounds_ptr = 0;
        ...
        sym_data = get_sym_ref(&char_pointer_type,
                               lbounds_section,
                               func_bound_offset,
