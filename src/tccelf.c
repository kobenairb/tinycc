...
ST_FUNC void relocate_section(TCCState *s1, Section *s)
{
    ...
    for_each_elem(sr, 0, rel, ElfW_Rel)
    {
        ...
        case R_ARM_THM_JUMP24: {
            ...
            (*(uint16_t *) (ptr + 2)) = (uint16_t) ((lo & 0xc000) | (j1 << 13) | blx_bit
                                                    | (j2 << 11) | imm11);
        } break;
        case R_ARM_MOVT_ABS:
        ...
        case R_AARCH64_MOVW_UABS_G0_NC:
            *(uint32_t *) ptr = (*(uint32_t *) ptr & 0xffe0001f) | (val & 0xffff) << 5;
            break;
        case R_AARCH64_MOVW_UABS_G1_NC:
            *(uint32_t *) ptr = (*(uint32_t *) ptr & 0xffe0001f) | (val >> 16 & 0xffff) << 5;
            break;
        case R_AARCH64_MOVW_UABS_G2_NC:
            *(uint32_t *) ptr = (*(uint32_t *) ptr & 0xffe0001f) | (val >> 32 & 0xffff) << 5;
            break;
        case R_AARCH64_MOVW_UABS_G3:
            *(uint32_t *) ptr = (*(uint32_t *) ptr & 0xffe0001f) | (val >> 48 & 0xffff) << 5;
            break;
        case R_AARCH64_ADR_PREL_PG_HI21: {
            uint64_t off = (val >> 12) - (addr >> 12);
            if ((off + ((uint64_t) 1 << 20)) >> 21)
                tcc_error("R_AARCH64_ADR_PREL_PG_HI21 relocation failed");
            *(uint32_t *) ptr = (*(uint32_t *) ptr & 0x9f00001f) | (off & 0x1ffffc) << 3
                                | (off & 3) << 29;
            break;
        }
        case R_AARCH64_ADD_ABS_LO12_NC:
            *(uint32_t *) ptr = (*(uint32_t *) ptr & 0xffc003ff) | (val & 0xfff) << 10;
            break;
        ...
        case R_AARCH64_CALL26:
           ...
           by section offset of course).  */
            ...
            if (((val - addr) + ((uint64_t) 1 << 27)) & ~(uint64_t) 0xffffffc) {
                tcc_error("R_AARCH64_(JUMP|CALL)26 relocation failed (val=%lx, addr=%lx)",
                          addr,
                          val);
            }
        ...
        case R_AARCH64_ADR_GOT_PAGE: {
            uint64_t off = (((s1->got->sh_addr + s1->sym_attrs[sym_index].got_offset) >> 12)
                            - (addr >> 12));
            if ((off + ((uint64_t) 1 << 20)) >> 21)
        ...
        case R_AARCH64_LD64_GOT_LO12_NC:
            *(uint32_t *) ptr = (*(uint32_t *) ptr & 0xfff803ff)
                                | ((s1->got->sh_addr + s1->sym_attrs[sym_index].got_offset) & 0xff8)
                                      << 7;
            break;
        case R_AARCH64_COPY:
            break;
        ...
        case R_X86_64_GOTPCREL:
            *(int *) ptr += (s1->got->sh_addr - addr + s1->sym_attrs[sym_index].got_offset - 4);
            break;
    ...
    for (i = 1; i < s1->nb_sections; i++) {
        ...
        for_each_elem(s, 0, rel, ElfW_Rel)
        {
            ...
            case R_ARM_PLT32:
                ...
                sym_index = ELFW(R_SYM)(rel->r_info);
                sym = &((ElfW(Sym) *) symtab_section->data)[sym_index];
                if (type != R_ARM_GOTOFF && type != R_ARM_GOTPC && sym->st_shndx == SHN_UNDEF) {
                    unsigned long ofs;
                    /* look at the symbol got offset. If none, then add one */
                    if (type == R_ARM_GOT32)
...
ST_FUNC void relocate_plt(TCCState *s1)
{
    ...
    if (p < p_end) {
        ...
        while (p < p_end) {
            ...
            uint64_t addr = got + (get32(p) | (uint64_t) get32(p + 4) << 32);
