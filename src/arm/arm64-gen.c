...
static int arm64_encode_bimm64(uint64_t x)
{
    ...
    return ((0x1000 & rep << 6) | (((rep - 1) ^ 31) << 1 & 63) | ((rep - pos) & (rep - 1)) << 6
            | (len - 1));
}
...
static void arm64_ldrx(int sg, int sz, int dst, int bas, uint64_t off)
{
    if (sz >= 2)
        sg = 0;
    if (!(off & ~(0xfff << sz)))
        o(0x39400000 | dst | bas << 5 | off << (10 - sz) | !!sg << 23
          | sz << 30); // ldr(*) x(dst),[x(bas),#(off)]
    else if (off < 256 || -off <= 256)
        o(0x38400000 | dst | bas << 5 | (off & 511) << 12 | !!sg << 23
          | sz << 30); // ldur(*) x(dst),[x(bas),#(off)]
    else {
        arm64_movimm(30, off); // use x30 for offset
        o(0x38206800 | dst | bas << 5 | 30 << 16 | (!!sg + 1) << 22
...
ST_FUNC void load(int r, SValue *sv)
{
    ...
    if (svr == (VT_LOCAL | VT_LVAL)) {
        if (IS_FREG(r))
            ...
            arm64_ldrx(!(svtt & VT_UNSIGNED), arm64_type_size(svtt), intr(r), 29, svcul);
        return;
    }
    ...
    if (svr == VT_CONST) {
        if ((svtt & VT_BTYPE) != VT_VOID)
            arm64_movimm(intr(r), arm64_type_size(svtt) == 3 ? sv->c.ull : (uint32_t) svcul);
        return;
    }
...
ST_FUNC void gfunc_call(int nb_args)
{
...

    {
        ...
        if (bt == VT_BYTE || bt == VT_SHORT)
            ...
            o(0x13001c00 | (bt == VT_SHORT) << 13 | !!(rt & VT_UNSIGNED) << 30); // [su]xt[bh] w0,w0
...
ST_FUNC void gfunc_prolog(CType *func_type)
{
    ...
    for (i = 1, sym = func_type->ref->next; sym; i++, sym = sym->next) {
        ...
        if (16 <= a[i] && a[i] < 32 && (sym->type.t & VT_BTYPE) == VT_STRUCT) {
            ...
            if (sz < 16)
                for (j = 0; j < k; j++) {
                    o(0x3d0003e0 | -(sz & 8) << 27 | (sz & 4) << 29 | ((a[i] - 16) / 2 + j)
                      | (off / sz + j) << 10);
                    // str ([sdq])(*),[sp,#(j * sz)]
                }
        }
...
ST_FUNC void gen_va_arg(CType *t)
{
    ...
    if (!hfa) {
        ...
        o(0x14000000); // b lab2
        // lab1:
        *(uint32_t *) (cur_text_section->data + b1) = (0x5400000d | (ind - b1) << 3);
        ...
        // lab2:
        *(uint32_t *) (cur_text_section->data + b2) = (0x14000000 | (ind - b2) >> 2);
    }
}
...
ST_FUNC void greturn(void)
{
    ...
    case 16:
        if ((func_vt.t & VT_BTYPE) == VT_STRUCT) {
            ...
            gv(RC_R(0));
            for (j = 0; j < n; j++)
                o(0x3d400000 | (sz & 16) << 19 | -(sz & 8) << 27 | (sz & 4) << 29 | j
...
ST_FUNC void gfunc_epilog(void)
{
    if (loc) {
        ...
        uint32_t *ptr = (uint32_t *) (cur_text_section->data + arm64_func_sub_sp_offset);
...
ST_FUNC void gen_cvt_itof(int t)
{
    if (t == VT_LDOUBLE) {
        ...
        ++vtop;
        vtop[0].r = d;
        o(0x1e220000 | !s << 16 | (t != VT_FLOAT) << 22 | fltr(d) | l << 31
...
ST_FUNC void gen_cvt_ftoi(int t)
{
    if ((vtop->type.t & VT_BTYPE) == VT_LDOUBLE) {
        ...
        ++vtop;
        vtop[0].r = d;
        o(0x1e380000 | !!(t & VT_UNSIGNED) << 16 | ((t & VT_BTYPE) == VT_LLONG) << 31 | intr(d)
...
ST_FUNC void gen_clear_cache(void)
{
    ...
    *(uint32_t *) (cur_text_section->data + b1) = (0x14000000 | (ind - b1) >> 2);
