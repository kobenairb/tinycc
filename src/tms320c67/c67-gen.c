...
void C67_asm(char *s, int a, int b, int c)
{
    ...
    if (strstr(s, "MVKL") == s) {
        C67_g((C67_map_regn(b) << 23) | ((a & 0xffff) << 7) | (0x0a << 2) | (C67_map_regs(b) << 1));
        ...
        C67_g((0 << 29) |               //creg
              ...
              (C67_map_regn(c) << 23) | //dst
              ((C67_map_regn(b) + 1) << 18)
              |                        //src2   WEIRD CPU must specify odd reg for some reason
              (0 << 13) |              //src1 NA
              (xpath << 12) |          //x cross path if opposite sides
              (0x1 << 5) |             //opcode
              (0x6 << 2) |             //opcode fixed
...
void load(int r, SValue *sv)
{
    ...
    if (fr & VT_LVAL) {
        ...
        if (v == VT_CONST) {
            ...
            t = v & 1;
            C67_B_DISP(
                4); //  Branch with constant displacement, skip over this branch, load, nop, load
    ...
    if ((vtop->r & (VT_VALMASK | VT_LVAL)) == VT_CONST) {
        ...
        if (vtop->r & VT_SYM) {
            ...
            if (is_jmp) {
                ...
                sym = get_sym_ref(&char_pointer_type,
                                  cur_text_section,
                                  ind + 12,
...
ST_FUNC int gfunc_sret(
    CType *vt, int variadic, CType *ret, int *ret_align, int *regsize, RegArgs *args)
{
...
void gfunc_prolog(CType *func_type)
{
    ...
    for (i = 0; i < NoOfCurFuncArgs; i++) {
        ...
        if (TranslateStackToReg[i] == 8) {
            C67_STW_PTR_PRE_INC(TREG_C67_A4 + i * 2 + 1,
                                C67_SP,
...
int gtst(int inv, int t)
{
    ...
    if (v == VT_CMP) {
        ...
        if (C67_compare_reg != TREG_EAX && // check if not already in a conditional test reg
            C67_compare_reg != TREG_EDX && C67_compare_reg != TREG_ST0
...
void gen_cvt_ftof(int t)
{
    ...
    } else if ((vtop->type.t & VT_BTYPE) == VT_FLOAT && (t & VT_BTYPE) == VT_DOUBLE) {
