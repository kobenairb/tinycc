...
static void tok_str_add2(TokenString *s, int t, CValue *cv)
{
    ...
    case TOK_LSTR: {
        ...
        memcpy((char *) (str + len) + sizeof(CString), cv->cstr->data, cstr.size);
        len += nb_words;
    } break;
    case TOK_CDOUBLE:
    case TOK_CLLONG:
...
static void pp_line(TCCState *s1, BufferedFile *f, int level)
{
    ...
    if (s1->Pflag == LINE_MACRO_OUTPUT_FORMAT_NONE || (level == 0 && f->line_ref && d < 8)) {
        while (d > 0)
            fputs("\n", s1->ppfp), --d;
...
static int macro_subst_tok(TokenString *tok_str, Sym **nested_list, Sym *s, int can_read_stream)
{
    Sym *args, *sa, *sa1;
    ...
    for (ptr = ptr0;;) {
        ...
        while (*ptr == TOK_TWOSHARPS) {
            ...
            while ((t1 = *++ptr) == TOK_NOSUBST)
                ;
            if (t1 && t1 != TOK_TWOSHARPS && t1 != ':') /* 'a##:' don't build a new token */
            {
                TOK_GET(&t1, &ptr, &cv1);
                        ...
                        int can_read_stream)
{
    ...
    while (1) {
        ...
        if (t >= TOK_IDENT && 0 == nosubst) {
            ...
            spc = (tok_str->len && is_space(tok_last(tok_str->str, tok_str->str + tok_str->len)));
...
ST_FUNC void preprocess_init(TCCState *s1)
{
    ...
    isidnum_table['$' - CH_EOF] = tcc_state->dollars_in_identifiers ? IS_ID : 0;
}
